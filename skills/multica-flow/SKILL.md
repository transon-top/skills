...
1. 查池子：`multica issue list --output json`
   - assignee 不明确时：`multica agent list --output json` 确认
2. 打开 issue：
   - `multica issue get <id> --output json` 拿全文
   - 评论两段有界读：`comment list <id> --roots-only --summary --output json` 扫根；命中相关线程才用 `--thread <thread-id> --tail <n>` 展开
...
4. **审定义**（派单前质量门，三问）：
   ...
   不够 → 回 3；够 → 5
5. **指派**：
   - 新任务：填 spec（按 `references/task-spec.template.md`，写 workdir 内）→ `multica issue create --title <一句话> --description-file <spec> --allow-external-file --assignee-id <agent-uuid>`
   - 用名字也行：`--assignee <name>` 模糊匹配
   - 状态：不传 `--status` 默认 `todo` 即入队；`--status backlog` 停车
   - `--project`/`--priority` 按工作区惯例传
   - 已有任务重新打磨：`multica issue update --description-file <spec>` 更新 → comment `[@agent](mention://agent/<uuid>)` 触发
...
6. 读交付：
   - final comment：`multica issue comment list`（变更清单/验证结果/PR URL）
   - PR 状态：`multica issue pull-requests <id>`。读前加载 `multica-working-on-issues`：state 单枚举 merged/closed/draft/open，`reference_only` 隐藏链接，`checks_conclusion` 看 CI。勿凭分支名或记忆推断
   - 声称与直觉不符：`multica issue runs <id>` 对执行历史（失败/重跑/中断能解释声称）
...
9. 质疑发回：
   ...
   - 发布后必读 `trigger_outcomes`，按 reason_code 分流：
     ...
     - `blocked` + `target_unavailable`/`runtime_offline` → agent 归档或失联：查 runtime，必要时 `update --assignee-id` 换 agent。重发无意义
