...
4. **审定义**（派单前质量门，三问）：
   ...
   不够 → 回 3；够 → 5
5. **指派（确认后执行）**：
   - 新任务：填 spec（按 `references/task-spec.template.md`，写 workdir 内）→ 派单前检查：spec 全文不含 `references/` 等技能内部路径、格式约束已内联自足 → 罗列开工清单（title、assignee、status、project/priority）→ 用户确认后 `multica issue create --title <一句话> --description-file <spec> --allow-external-file --assignee-id <agent-uuid> --profile <name>`
