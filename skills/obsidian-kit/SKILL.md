...
- 表述为通用方法与最佳实践；不出现项目路径、业务专有名词
- 记录机制与原理, 不记录依赖安装/复用方法; 不出现**任何文件/目录路径与第三方依赖名** (库、包、框架名, 含 node_modules 与包内模块路径); 语言层标准 API/技术名可保留
- 参数不从记忆编造: 读源码/实现或文档查证机制与数值 (时长、延迟、公式等) 后, 脱敏提炼再落笔
- 代码为最小可复现 demo: 用语言层标准 API 或伪代码表达原理, 无依赖与项目上下文
